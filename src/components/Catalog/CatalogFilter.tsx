...
import {  getBaseLimit } from "../../Redux/slices/dataSlice";
...
const CatalogFilter = () => {
  ...
  const handleSizeChange = (size: number) => {
    ...
    const onSubmit: SubmitHandler<Filter> = (data) => {
      ...
      const closeModal = () =>{
        const overlayFilter = document.getElementById('overlayFilter') as HTMLDialogElement;
        if (overlayFilter) {
            overlayFilter.close();
        }
      }
    return ( 
        <form className={style.selection} method="dialog" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button className={style.BtnCloseModal} onClick={closeModal}>&#10006;</button>
        </form>
     );
}
